...
CREATE TABLE cortst.level2_locations (
    id bigint NOT NULL,
    store_id bigint,
...
ALTER TABLE cortst.level2_locations OWNER TO fastinfo_dba;
CREATE SEQUENCE cortst.level2_locations_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1;
ALTER TABLE cortst.level2_locations_id_seq OWNER TO fastinfo_dba;
ALTER SEQUENCE cortst.level2_locations_id_seq OWNED BY cortst.level2_locations.id;
ALTER TABLE ONLY cortst.level2_locations ALTER COLUMN id SET DEFAULT nextval('cortst.level2_locations_id_seq'::regclass);
ALTER TABLE ONLY cortst.level2_locations
    ADD CONSTRAINT level2_locations_pkey PRIMARY KEY (id);
CREATE UNIQUE INDEX index_level2_locations_on_loc_tran_loc ON cortst.level2_locations USING btree (loc_tran_loc);
CREATE INDEX index_level2_locations_on_location_type_id ON cortst.level2_locations USING btree (location_type_id);
CREATE INDEX index_level2_locations_on_row_creator_id ON cortst.level2_locations USING btree (row_creator_id);
CREATE INDEX index_level2_locations_on_store_id ON cortst.level2_locations USING btree (store_id);
