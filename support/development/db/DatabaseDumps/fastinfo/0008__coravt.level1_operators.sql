...
CREATE TABLE coravt.level1_operators ( 
    id bigint NOT NULL, 
...
ALTER TABLE coravt.level1_operators OWNER TO fastinfo_dba; 
CREATE SEQUENCE coravt.level1_operators_id_seq 
    START WITH 1 
    INCREMENT BY 1 
    NO MINVALUE 
    NO MAXVALUE 
    CACHE 1; 
ALTER TABLE coravt.level1_operators_id_seq OWNER TO fastinfo_dba; 
ALTER SEQUENCE coravt.level1_operators_id_seq OWNED BY coravt.level1_operators.id; 
ALTER TABLE ONLY coravt.level1_operators ALTER COLUMN id SET DEFAULT nextval('coravt.level1_operators_id_seq'::regclass); 
ALTER TABLE ONLY coravt.level1_operators 
    ADD CONSTRAINT level1_operators_pkey PRIMARY KEY (id); 
CREATE INDEX index_level1_operators_on_ht_etl_processing_stage ON coravt.level1_operators USING btree (ht_etl_processing_stage) WHERE (ht_etl_processing_stage < 10); 
