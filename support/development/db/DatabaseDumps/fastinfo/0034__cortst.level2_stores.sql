...
CREATE TABLE cortst.level2_stores (
    id bigint NOT NULL,
...
ALTER TABLE cortst.level2_stores OWNER TO fastinfo_dba;
CREATE SEQUENCE cortst.level2_stores_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1;
ALTER TABLE cortst.level2_stores_id_seq OWNER TO fastinfo_dba;
ALTER SEQUENCE cortst.level2_stores_id_seq OWNED BY cortst.level2_stores.id;
ALTER TABLE ONLY cortst.level2_stores ALTER COLUMN id SET DEFAULT nextval('cortst.level2_stores_id_seq'::regclass);
ALTER TABLE ONLY cortst.level2_stores
    ADD CONSTRAINT level2_stores_pkey PRIMARY KEY (id);
CREATE UNIQUE INDEX index_level2_stores_on_loc_tran_loc ON cortst.level2_stores USING btree (loc_tran_loc);
CREATE INDEX index_level2_stores_on_row_creator_id ON cortst.level2_stores USING btree (row_creator_id);
