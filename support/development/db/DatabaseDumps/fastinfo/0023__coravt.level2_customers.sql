...
ALTER TABLE coravt.level2_customers OWNER TO postgres;
CREATE SEQUENCE level2_customers_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1;
ALTER TABLE coravt.level2_customers_id_seq OWNER TO postgres;
ALTER SEQUENCE level2_customers_id_seq OWNED BY level2_customers.id;
ALTER TABLE ONLY level2_customers ALTER COLUMN id SET DEFAULT nextval('level2_customers_id_seq'::regclass);
ALTER TABLE ONLY level2_customers
    ADD CONSTRAINT level2_customers_pkey PRIMARY KEY (id);
CREATE UNIQUE INDEX index_level2_customers_on_cust_acct_nbr ON level2_customers USING btree (cust_acct_nbr);
CREATE INDEX index_level2_customers_on_cust_first_name_mi ON level2_customers USING btree (cust_first_name_mi);
CREATE INDEX index_level2_customers_on_cust_last_name ON level2_customers USING btree (cust_last_name);
CREATE INDEX index_level2_customers_on_default_store_id ON level2_customers USING btree (default_store_id);
CREATE INDEX index_level2_customers_on_row_creator_id ON level2_customers USING btree (row_creator_id);
