...
CREATE TABLE coravt.level2_locations ( 
    id bigint NOT NULL, 
    store_id bigint, 
...
ALTER TABLE coravt.level2_locations OWNER TO fastinfo_dba; 
CREATE SEQUENCE coravt.level2_locations_id_seq 
    START WITH 1 
    INCREMENT BY 1 
    NO MINVALUE 
    NO MAXVALUE 
    CACHE 1; 
ALTER TABLE coravt.level2_locations_id_seq OWNER TO fastinfo_dba; 
ALTER SEQUENCE coravt.level2_locations_id_seq OWNED BY coravt.level2_locations.id; 
ALTER TABLE ONLY coravt.level2_locations ALTER COLUMN id SET DEFAULT nextval('coravt.level2_locations_id_seq'::regclass); 
ALTER TABLE ONLY coravt.level2_locations 
    ADD CONSTRAINT level2_locations_pkey PRIMARY KEY (id); 
CREATE UNIQUE INDEX index_level2_locations_on_loc_tran_loc ON coravt.level2_locations USING btree (loc_tran_loc); 
CREATE INDEX index_level2_locations_on_location_type_id ON coravt.level2_locations USING btree (location_type_id); 
CREATE INDEX index_level2_locations_on_row_creator_id ON coravt.level2_locations USING btree (row_creator_id); 
CREATE INDEX index_level2_locations_on_store_id ON coravt.level2_locations USING btree (store_id); 
