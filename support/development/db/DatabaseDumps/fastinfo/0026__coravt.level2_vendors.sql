...
CREATE TABLE coravt.level2_vendors ( 
    id bigint NOT NULL, 
...
ALTER TABLE coravt.level2_vendors OWNER TO fastinfo_dba; 
CREATE SEQUENCE coravt.level2_vendors_id_seq 
    START WITH 1 
    INCREMENT BY 1 
    NO MINVALUE 
    NO MAXVALUE 
    CACHE 1; 
ALTER TABLE coravt.level2_vendors_id_seq OWNER TO fastinfo_dba; 
ALTER SEQUENCE coravt.level2_vendors_id_seq OWNED BY coravt.level2_vendors.id; 
ALTER TABLE ONLY coravt.level2_vendors ALTER COLUMN id SET DEFAULT nextval('coravt.level2_vendors_id_seq'::regclass); 
ALTER TABLE ONLY coravt.level2_vendors 
    ADD CONSTRAINT level2_vendors_pkey PRIMARY KEY (id); 
CREATE UNIQUE INDEX index_level2_vendors_on_vend_number ON coravt.level2_vendors USING btree (vend_number); 
