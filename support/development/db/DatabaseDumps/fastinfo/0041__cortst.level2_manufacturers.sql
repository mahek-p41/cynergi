...
CREATE TABLE cortst.level2_manufacturers (
    id bigint NOT NULL,
...
ALTER TABLE cortst.level2_manufacturers OWNER TO fastinfo_dba;
CREATE SEQUENCE cortst.level2_manufacturers_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
    CACHE 1;
ALTER TABLE cortst.level2_manufacturers_id_seq OWNER TO fastinfo_dba;
ALTER SEQUENCE cortst.level2_manufacturers_id_seq OWNED BY cortst.level2_manufacturers.id;
ALTER TABLE ONLY cortst.level2_manufacturers ALTER COLUMN id SET DEFAULT nextval('cortst.level2_manufacturers_id_seq'::regclass);
ALTER TABLE ONLY cortst.level2_manufacturers
    ADD CONSTRAINT level2_manufacturers_pkey PRIMARY KEY (id);
CREATE UNIQUE INDEX index_level2_manufacturers_on_manu_code ON cortst.level2_manufacturers USING btree (manu_code);
CREATE INDEX index_level2_manufacturers_on_row_creator_id ON cortst.level2_manufacturers USING btree (row_creator_id);
