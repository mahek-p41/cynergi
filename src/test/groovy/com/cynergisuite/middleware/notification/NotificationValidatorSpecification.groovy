...
class NotificationValidatorSpecification extends Specification {
   ...
   void "validate save valid NotificationValueObject of type All" () {
      ...
      final notificationTypeAll = NotificationTypeDomainTestDataLoader.values().find { it.value == "A" }

      when:
      final notificationValueObject = NotificationTestDataLoader.stream(1, "corrto", null, null, notificationTypeAll, null)
         .map { new NotificationValueObject(it) }
         .findFirst().orElseThrow { new Exception("Unable to create Notification") }
      new NotificationValidator(notificationService).validateCreate(notificationValueObject)
   ...
   void "validate save valid NotificationValueObject of type Employee with single recipient" () {
      given:
      final NotificationService notificationService = Mock()
      final notificationTypeAll = NotificationTypeDomainTestDataLoader.values().find { it.value == "E" }

      when:
      final notification = NotificationTestDataLoader.stream(1, "corrto", null, null, notificationTypeAll, null).findFirst().orElseThrow { new Exception("Unable to create Notification") }
      final notificationRecipients = NotificationRecipientTestDataLoader.stream(1, notification).collect(Collectors.toList())
      final notificationValueObject = new NotificationValueObject(notification, notificationRecipients)
      new NotificationValidator(notificationService).validateCreate(notificationValueObject)
   ...
   void "validate save invalid NotificationValueObject of type All where startDate is after expirationDate" () {
      ...
      when:
      final notificationValueObject = NotificationTestDataLoader.stream(1, "corrto", LocalDate.of(2000, FEBRUARY, 2), LocalDate.of(2000, JANUARY, 2), notificationTypeAll, null)
         .map { new NotificationValueObject(it) }
         .findFirst().orElseThrow { new Exception("Unable to create Notification") }
      new NotificationValidator(notificationService).validateCreate(notificationValueObject)
   ...
   void "validate update valid NotificationValueObject of type all" () {
      ...
      final notificationValueObject = NotificationTestDataLoader.stream(1, "corrto", null, null, notificationTypeAll, null)
         ...
         .findFirst().orElseThrow { new Exception("Unable to create Notification") }
      new NotificationValidator(notificationService).validateUpdate(notificationValueObject)
