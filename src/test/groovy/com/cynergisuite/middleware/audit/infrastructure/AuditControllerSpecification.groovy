...
class AuditControllerSpecification extends ControllerSpecificationBase {
   ...
   void "create new audit when previous audit was approved" () {
      ...
      final employee = employeeFactoryService.single(company)
      final store = storeFactoryService.random(company)
      auditFactoryService.single(store, employee, [AuditStatusFactory.created(), AuditStatusFactory.canceled(), AuditStatusFactory.approved()] as Set)
   ...
   void "update completed audit to approved and approve exceptions" () {
      ...
      when:
      def result = put("$path/approved", new AuditUpdateValueObject(['id': audit.id, 'status': new AuditStatusValueObject([value: 'APPROVED'])]))
   ...
   void "Confirm exceptions approved when audit is approved" () {
      ...
      when:
      put("$path/approved", new AuditUpdateValueObject([id: auditOne.id, status: new AuditStatusValueObject([value: "APPROVED"])]))
   ...
   void "approve all audit exceptions" () {
      ...
      when:
      def result = put("$path/approved/exceptions", new SimpleIdentifiableDataTransferObject(audit.myId()))
