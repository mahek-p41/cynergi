...
(
    id                INTEGER                                                       NOT NULL PRIMARY KEY,
    parent_id         INTEGER REFERENCES menu_type_domain (id),
    value             VARCHAR(50) CHECK (CHAR_LENGTH(TRIM(value)) > 1)              NOT NULL,
    description       VARCHAR(100) CHECK (CHAR_LENGTH(TRIM(description)) > 1)       NOT NULL,
    localization_code VARCHAR(100) CHECK (CHAR_LENGTH(TRIM(localization_code)) > 1) NOT NULL,
...
VALUES (1, null, 'ACCOUNTS_PAYABLE', 'Account Payable', 'account.payable', (SELECT id FROM area_type_domain WHERE value = 'AP'), 0),
       (2, null, 'INVOICE_MAINTENANCE', 'AP Invoice', 'account.payable.invoice.maintenance', (SELECT id FROM area_type_domain WHERE value = 'AP'), 1),
       (3, null, 'RECURRING_INVOICE_MAINTENANCE', 'Recurring Invoice', 'recurring.invoice.maintenance', (SELECT id FROM area_type_domain WHERE value = 'AP'), 2),
       (4, null, 'CHECK_MAINTENANCE', 'AP Check', 'account.payable.check.maintenance', (SELECT id FROM area_type_domain WHERE value = 'AP'), 3),
       ...
       (8, null, 'STORE_DEPOSIT_MAINTENANCE', 'Store Deposit', 'store.deposit.maintenance', (SELECT id FROM area_type_domain WHERE value = 'BR'), 1),
       ...
       (10, null, 'CLEAR_OUTSTANDING_ITEMS', 'Outstanding Items', 'outstanding.items.maintenance', (SELECT id FROM area_type_domain WHERE value = 'BR'), 3),
       ...
       (12, null, 'BANK_TRANSACTIONS_MAINTENANCE', 'Bank Transactions', 'bank.transactions.maintenance', (SELECT id FROM area_type_domain WHERE value = 'BR'), 5),
...
VALUES (1, 'APADD', 'Add Invoices', 'add.invoices', 'APADD', (SELECT id FROM menu_type_domain WHERE value = 'INVOICE_MAINTENANCE')),
       ...
       (34, 'ITEMMNTS', 'Model', 'model.maintenance', 'ITEMMNTS', (SELECT id FROM menu_type_domain WHERE value = 'PO_MAINTENANCE')),
       (35, 'VENDOR', 'Vendor', 'vendor.maintenance', 'VENDOR', (SELECT id FROM menu_type_domain WHERE value = 'PO_MAINTENANCE')),
