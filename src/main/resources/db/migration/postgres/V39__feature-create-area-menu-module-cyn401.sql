...
(
    id                INTEGER                                                       NOT NULL PRIMARY KEY,
    parent_id         BIGINT REFERENCES menu_type_domain (id)                               ,
    value             VARCHAR(50) CHECK (char_length(trim(value)) > 1)              NOT NULL,
    description       VARCHAR(100) CHECK (char_length(trim(description)) > 1)       NOT NULL,
    localization_code VARCHAR(100) CHECK (char_length(trim(localization_code)) > 1) NOT NULL,
    area_type_id      BIGINT REFERENCES area_type_domain (id)                               ,
    order_number     INTEGER DEFAULT 0,
    UNIQUE (area_type_id, value)
);

INSERT INTO menu_type_domain (id, parent_id, value, description, localization_code, area_type_id, order_number)
VALUES (1, null, 'ACCOUNTS_PAYABLE', 'Account Payable', 'account.payable', (SELECT id FROM area_type_domain WHERE value = 'AP'), 0),
       (2, null, 'INVOICE_MAINTENANCE', 'AP Invoice Maintenance', 'account.payable.invoice.maintenance', (SELECT id FROM area_type_domain WHERE value = 'AP'), 1),
       (3, null, 'RECURRING_INVOICE_MAINTENANCE', 'Recurring Invoice Maintenance', 'recurring.invoice.maintenance', (SELECT id FROM area_type_domain WHERE value = 'AP'), 2),
       (4, null, 'CHECK_MAINTENANCE', 'AP Check Maintenance', 'account.payable.check.maintenance', (SELECT id FROM area_type_domain WHERE value = 'AP'), 3),
       (5, null, 'AP_REPORTS', 'AP Reports', 'account.payable.reports', (SELECT id FROM area_type_domain WHERE value = 'AP'), 4),
       (6, null, 'MONTH_END', 'Account Payable Month End', 'account.payable.month.end', (SELECT id FROM area_type_domain WHERE value = 'AP'), 5),

       (7, null, 'BANK_RECONCILIATION', 'Bank Reconciliation', 'bank.reconciliation', (SELECT id FROM area_type_domain WHERE value = 'BR'), 0),
       (8, null, 'STORE_DEPOSIT_MAINTENANCE', 'Store Deposit Maintenance', 'store.deposit.maintenance', (SELECT id FROM area_type_domain WHERE value = 'BR'), 1),
       (9, null, 'SEND_STORE_DEPOSIT_TO_BANK_REC', 'Send Store Deposit to Bank Rec', 'send.store.deposit.to.bank.rec', (SELECT id FROM area_type_domain WHERE value = 'BR'), (SELECT id FROM area_type_domain WHERE value = 'BR')),
       (10, null, 'CLEAR_OUTSTANDING_ITEMS', 'Outstanding Items Maintenance', 'outstanding.items.maintenance', (SELECT id FROM area_type_domain WHERE value = 'BR'), 3),
       (11, null, 'RECONCILE_BANK_ACCOUNT', 'Reconcile Bank Account', 'reconcile.bank.account', (SELECT id FROM area_type_domain WHERE value = 'BR'), 4),
       (12, null, 'BANK_TRANSACTIONS_MAINTENANCE', 'Bank Transactions Maintenance', 'bank.transactions.maintenance', (SELECT id FROM area_type_domain WHERE value = 'BR'), 5),

       (13, null, 'GENERAL_LEDGER', 'General Ledger', 'general.ledger', (SELECT id FROM area_type_domain WHERE value = 'GL'), 0),
       (14, null, 'ACCOUNT_INQUIRY_ANALYSIS', 'GL Inquiry/Analysis', 'general.ledger.inquiry.analysis', (SELECT id FROM area_type_domain WHERE value = 'GL'), 1),
       (15, null, 'JOURNAL_ENTRY PROCESSING', 'Journal Entry Processing', 'journal.entry.processing', (SELECT id FROM area_type_domain WHERE value = 'GL'), 2),
       (16, null, 'GL_REPORTS', 'GL Reports', 'general.ledger.reports', (SELECT id FROM area_type_domain WHERE value = 'GL'), (SELECT id FROM area_type_domain WHERE value = 'GL')),
       (17, null, 'MONTH_END_PROCESSING', 'GL End of Month Processing', 'general.ledger.end.of.month.processing', (SELECT id FROM area_type_domain WHERE value = 'GL'), 4),
       (18, null, 'UTILITIES', 'GL Utilities', 'general.ledger.utilities', (SELECT id FROM area_type_domain WHERE value = 'GL'), 5),

       (20, null, 'PO_MAINTENANCE', 'PO Maintenance', 'purchase.order.maintenance', (SELECT id FROM area_type_domain WHERE value = 'PO'), 1),
       ...
       (29, 28, 'BANK', 'Bank Maintenance', 'bank.maintenance', (SELECT id FROM area_type_domain WHERE value = 'MCF'), 2),
       (30, 28, 'CHART_OF_ACCOUNT', 'Chart of Account', 'chart.of.account', (SELECT id FROM area_type_domain WHERE value = 'MCF'), 3),
       (31, 28, 'DISTRIBUTION_TEMPLATE_MAINTENANCE', 'Distribution Template Maintenance', 'distribution.template.maintenance', (SELECT id FROM area_type_domain WHERE value = 'MCF'), 4),
       ...
       (34, 28, 'SHIPVIA', 'Ship Via Maintenance', 'ship.via.maintenance', (SELECT id FROM area_type_domain WHERE value = 'MCF'), 7),
       (35, 28, 'VENDOR', 'Vendor Maintenance', 'vendor.maintenance', (SELECT id FROM area_type_domain WHERE value = 'MCF'), 8),
       ...
       (38, 37, 'COMPANY', 'Company Maintenance', 'company.maintenance', (SELECT id FROM area_type_domain WHERE value = 'MCF'), 11),
       ...
       (41, 37, 'PO_CONTROL', 'PO Control', 'po.control', (SELECT id FROM area_type_domain WHERE value = 'MCF'), 14),
       (42, 37, 'GL_CONTROL', 'GL Control', 'general.ledger.control', (SELECT id FROM area_type_domain WHERE value = 'MCF'), 15)
;
...
VALUES (1, 'APADD', 'Add Invoices', 'add.invoices', 'APADD', (SELECT id FROM menu_type_domain WHERE value = 'INVOICE_MAINTENANCE')),
       ...
       (46, 'GETSTKLV', 'Update Stock Reorder Control File', 'update.stock.reorder.control.file', 'GETSTKLV', (SELECT id FROM menu_type_domain WHERE value = 'STOCK_REORDER')),
