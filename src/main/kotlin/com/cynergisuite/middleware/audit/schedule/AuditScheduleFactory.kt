...
object AuditScheduleFactory {
   ...
   fun stream(numberIn: Int = 1, dayOfWeekIn: DayOfWeek? = null, storeIn: StoreEntity? = null, departmentAccessIn: DepartmentEntity? = null) : Stream<AuditScheduleEntity> {
      ...
      val schedule = ScheduleEntity(
         id = null,
         title = "${dayOfWeek.getDisplayName(TextStyle.FULL, Locale.US)} Store Audit",
         description = "${dayOfWeek.getDisplayName(TextStyle.FULL, Locale.US)} Store Audit",
...
class AuditScheduleFactoryService(
   ...
   fun stream(numberIn: Int = 1, dayOfWeekIn: DayOfWeek? = null, storeIn: StoreEntity? = null, departmentAccessIn: DepartmentEntity? = null): Stream<AuditScheduleEntity> {
      ...
      return AuditScheduleFactory.stream(numberIn, dayOfWeekIn, store, departmentAccessIn)
