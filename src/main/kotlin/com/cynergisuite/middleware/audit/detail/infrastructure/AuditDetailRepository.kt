...
class AuditDetailRepository @Inject constructor(
   ...
   private val selectBase = """
      ...
      SELECT
         ...
         ad.inventory_status AS ad_inventory_status,
   ...
   override fun insert(entity: AuditDetail): AuditDetail {
      ...
      return jdbc.insertReturning("""
         INSERT INTO audit_detail(scan_area_id, bar_code, inventory_id, inventory_brand, inventory_model, scanned_by, inventory_status, audit_id)
         VALUES (:scan_area_id, :bar_code, :inventory_id, :inventory_brand, :inventory_model, :scanned_by, :inventory_status, :audit_id)
         RETURNING
            *
         """.trimIndent(),
         mapOf(
            ...
            "inventory_status" to entity.inventoryStatus,
   ...
   private fun mapRow(rs: ResultSet, scanArea: AuditScanArea, scannedBy: Employee, audit: IdentifiableEntity, columnPrefix: String = EMPTY): AuditDetail {
      return AuditDetail(
         ...
         scannedBy = scannedBy,
         inventoryStatus = rs.getString("${columnPrefix}inventory_status"),
         audit = audit
      )
   }
}
