...
class PendingVerifyStagingsForCurrentFiscalYear(from: LocalDate, thru: LocalDate) : Cynergi("cynergi.validation.unposted.verify.stagings.found.for.current.year", arrayOf(from, thru))
...
class CheckCleared(): Cynergi("cynergi.validation.check.cleared", emptyArray())
class CheckVoided(): Cynergi("cynergi.validation.check.voided", emptyArray())
