...
class InloadSUMGLINTVService @Inject constructor(
   ...
   override fun inloadCsv(record: CSVRecord, batchId: UUID) {
      ...
      val map: MutableMap<String, Any?> = mutableMapOf(
         ...
         "DEP-1" to Pair(record["Dep_Cash_Amt"].toBigDecimal(), record["Dep_Cash_GL_Acct_Nbr"].toLong()),
         "DEP-2" to Pair(record["Dep_For_Oth_Str_Amt"].toBigDecimal(), record["Dep_For_Oth_Str_GL_Acct_Nbr"].toLong()),
         "DEP-3" to Pair(record["Dep_From_Oth_Str_Amt"].toBigDecimal(), record["Dep_From_Oth_Str_GL_Acct_Nbr"].toLong()),
         "DEP-4" to Pair(record["Dep_CC_In_Str_Amt"].toBigDecimal(), record["Dep_CC_In_Str_GL_Acct_Nbr"].toLong()),
         "DEP-5" to Pair(record["Dep_ACH_OLP_Amt"].toBigDecimal(), record["Dep_ACH_OLP_GL_Acct_Nbr"].toLong()),
         "DEP-6" to Pair(record["Dep_CC_OLP_Amt"].toBigDecimal(), record["Dep_CC_OLP_GL_Acct_Nbr"].toLong()),
         "DEP-7" to Pair(record["Dep_Debit_Card_Amt"].toBigDecimal(), record["Dep_Debit_Card_GL_Acct_Nbr"].toLong()),
         "DEP-8" to Pair(record["ACH_Chargeback_Amt"].toBigDecimal(), record["ACH_Chargeback_GL_Acct_Nbr"].toLong()),
         "DEP-9" to Pair(record["ICC_Chargeback_Amt"].toBigDecimal(), record["ICC_Chargeback_GL_Acct_Nbr"].toLong()),
         "DEP-10" to Pair(record["NSF_Return_Check_Amt"].toBigDecimal(), record["NSF_Return_Check_GL_Acct_Nbr"].toLong()),
         "DEP-11" to Pair(record["AR_Bad_Check_Amt"].toBigDecimal(), record["AR_Bad_Check_GL_Acct_Nbr"].toLong()),
         "message" to null
      )
